...
interface HeroLineBackgroundProps {
  variant?: "default" | "growth" | "contact" | "process" | "solutions"
}

export default function HeroLineBackground({ variant = "default" }: HeroLineBackgroundProps) {
  const { scrollYProgress } = useScroll()
  const yGold = useTransform(scrollYProgress, [0, 0.2], [0, variant === "contact" ? 30 : 50])
  const yNavy = useTransform(scrollYProgress, [0, 0.2], [0, variant === "contact" ? -20 : -35])
  const opacity = variant === "contact" ? 0.06 : 0.1
